...
class Inference_Engine():
    ...
    def convert_PyTorch_to_TensorRT(self):
        ...
        # 3. 编译模型（启用 FP16 精度，对应旧版的 fp16_mode=True）
        model_trt = torch_tensorrt.compile(
            self.model,
            inputs=[torch_tensorrt.Input(x.shape)],
            enabled_precisions={torch.float16},  # 对应 FP16 加速
            workspace_size=1 << 25  # 可选：设置工作空间大小（512MB），根据模型调整
        )
